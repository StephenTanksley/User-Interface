...
# # We'll say that a positive int divides itself if every digit in the number divides into the number evently. The number 128 divides itself since 1, 2, 8 divides into 128 evenly. Write a function to determine if a number divides itself.


# """
# U - We'll say that a positive int divides itself if every digit in the number divides into the number evently. The number 128 divides itself since 1, 2, 8 divides into 128 evenly. Write a function to determine if a number divides itself.

# P -
#     Make an original copy of Num.
#     Loop over each digit in num (while the current number is not < 10):
#         get right-most digit of NUM using num % 10
#         if the digit is 0, return False.
#         take the digit and get remainder of original num / digit
...
def brute_force_fill_knapsack(sack, items):
    ...
    best_value = -1
    # for each combo, find the total weight and value of items. Save the best one.
    for combo in combos:
        value = 0
        weight = 0
        for item in combo:
        ...
        if weight <= 50 and value > best_value:
            best_value = value
            # this is the best value thus far
            sack = combo
